...
let DUNGA = function () {
                ...
                if (data.status == true) {
                    ...
                    if (data.data_momo) {
                        ...
                        for (let momo of data.data_momo) {
                            ...
                            table_one += `<tr>`;
                            table_one += `<td>${momo.username} <span class="label label-success text-uppercase" onclick="DUNGA.coppy('${momo.username}', '${momo.settings.min}', '${momo.settings.max}')"><i class="fa fa-clipboard" aria-hidden="true"></i></span> <span class="label label-success text-uppercase" onclick="DUNGA.play('${momo.username}')"><i class="fa fa-play" aria-hidden="true"></i></span></td>`;
    ...
    return {
        ...
        coppy: function (str, min, max) {
            copyStringToClipboard(str);
            alert('Đã sao chép số: ' + str + ' chơi từ ' + number_format(min) + ' VNĐ đến ' + number_format(max) + ' VNĐ. Nếu bạn chuyển nhỏ hơn hoặc lớn thua sẽ mất tiền.  ');
        },
        play: function (str) {
